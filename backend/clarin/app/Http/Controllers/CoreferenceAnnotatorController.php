    ...
    try {
      ...
      return Response::json(array(
                  ...
                  'data'    => CoreferenceAnnotator::where('user_id', $user['id'])
                                       ->first(array('language', 'annotation_type', 'alternative'))));
    }catch(\Exception $e){
  ...
  public function store() {    
    try {
      ...
      if (!$annotationSchemaExists->count()){       //annotation schema does not exist -- save new annotation schema
        $newAnnotationSchema = CoreferenceAnnotator::create(array(  
          'user_id' => $user['id'],    
        ...
        $newAnnotationSchema = CoreferenceAnnotator::where('user_id', '=',  $user['id'])->update(array(
          'language' => $input['language'],          
          'annotation_type' => $input['annotation_type'],
          'alternative' => $input['alternative']
        ));
