...
export class AnnotatorWidgetComponent extends BaseControlComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.TextWidgetAPI.registerAnnotationSchemaCallback(this.updateAnnotatorTemplate);
  }

  updateAnnotatorTemplate = function () {
    ...
    this.annotatorsTemplateService.getTemplate(this.annotatorType, this.annotationSchema)
      .then((annotatorsTemplate:any)=> {
        ...
        this.element.html('<div autoslimscroll scroll-subtraction-height="145">' + annotatorsTemplate + '</div>');
