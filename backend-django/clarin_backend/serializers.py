...
class MyTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def validate(self, attrs):
        ...
        try:
            ...
            if userr.exists():
                ...
                if check == False:
                    return {
                        "success": False,
                        "message": "No active account with these credentials",
                    }
                else:
                    ...
                    try:
                        ...
                        return {
                            "success": False,
                            "message": "An error occured. email has not been sent",
                        }

                    return {
                        "success": False,
                        "message": "The account  with these credentials has not been activated. Please look your email.",
                    }
            # print(check)
        ...
        data["data"] = {}
        data["data"]["jwtToken"] = {}
        data["data"]["jwtToken"]["refresh"] = data["refresh"]
        # str(refresh)
        data["data"]["jwtToken"]["access"] = data["access"]
...
class CollectionsSerializer(serializers.ModelSerializer):
    ...
    owner_id = serializers.PrimaryKeyRelatedField(
        read_only=False, queryset=Users.objects.all()
    )
...
class DocumentsSerializer(serializers.ModelSerializer):
    ...
    data_file = serializers.FileField(allow_null=True, default=None)
    ...
    collection_id = serializers.PrimaryKeyRelatedField(
        read_only=False, queryset=Collections.objects.all()
    )
...
class OpenDocumentsSerializer(serializers.ModelSerializer):
    ...
    document_id = serializers.PrimaryKeyRelatedField(
        read_only=False, queryset=Documents.objects.all()
    )
...
class SharedCollectionsSerializer(serializers.ModelSerializer):
    ...
    fromfield = serializers.SlugRelatedField(
        many=False, read_only=False, slug_field="email", queryset=Users.objects.all()
    )
    tofield = serializers.SlugRelatedField(
        many=False, read_only=False, slug_field="email", queryset=Users.objects.all()
    )
