angular.module('clarin-el').factory('TextWidgetAPI', function() {
    ...
    return {
        ...
        getAnnotationsToBeAdded: function() { return annotationsToBeAdded; },
        clearAnnotationsToBeAdded: function() { annotationsToBeAdded = []; },
        registerNewAnnotationsCallback: function(callback) { annotationsToBeAddedCallbacks.push(callback); },
        ...
        getAnnotationById: function (annotationId) {
            return  _.findWhere(annotations, {_id: annotationId});
        ...
        updateAnnotation: function(updatedAnnotation, selected) {
            ...
            annotationsToBeAdded.push({"annotation": updatedAnnotation, "selected": selected});
        ...
        deleteAnnotation: function(annotationId) {
            var deletedAnnotation = _.findWhere(annotations, {_id: annotationId});
        ...
        registerAnnotationsCallback: function(callback) { annotationsCallbacks.push(callback) },
        ...
        matchAnnotationsToSchema: function(newAnnotations) {      //match the document 's annotations to the annotation schema
			for (var i=0; i<newAnnotations.length; i++) {
				if (this.belongsToSchema(newAnnotations[i])) {
					if (angular.equals(annotatorType, "Button Annotator")) {
						for (var j=0; j<newAnnotations[i].attributes.length; j++) {
					...
					annotations.push(newAnnotations[i]);
					annotationsToBeAdded.push({"annotation": newAnnotations[i], "selected": false});
				}
			}
        ...
        registerCurrentDocumentCallback: function(callback) { currentDocumentCallbacks.push(callback); },
        getCurrentDocument: function() { return currentDocument; },
        ...
        getAnnotatorType: function() { return annotatorType; },
        setAnnotatorType: function(newAnnotatorType) { annotatorType = angular.copy(newAnnotatorType); },
        clearAnnotatorType: function() { annotatorType = {}; },
        ...
        setSelectedAnnotation: function(newSelectedAnnotation){
            if (angular.isUndefined(newSelectedAnnotation) || angular.equals(newSelectedAnnotation, {})) { return false; }
        ...
        setSelectedAnnotationById: function(annotationId){
            if (angular.isUndefined(annotationId)) { return false; }

/*
            if(!angular.equals(selectedAnnotation,{}))
                annotationsToBeAdded.push({"annotation": selectedAnnotation, "selected": false}); */

            var newSelectedAnnotation = _.findWhere(annotations, {_id: annotationId});
        ...
        getFoundInCollection : function() { return foundInCollection; },
        setFoundInCollection : function(newFoundInCollection) { foundInCollection=angular.copy(newFoundInCollection); },
        clearFoundInCollection : function() { foundInCollection = []; }
    }
});